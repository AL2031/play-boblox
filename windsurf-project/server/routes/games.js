...
router.post('/', authenticateToken, async (req, res, next) => {
  ...
  try {
    // Write directly to permanent storage for game creation
    const result = await new Promise((resolve, reject) => {
      storage.permanentDb.run(
        'INSERT INTO games (title, description, creator_id, code, published) VALUES (?, ?, ?, ?, ?)',
        [title, description, creatorId, code, published !== undefined ? (published ? 1 : 0) : 1],
        function(err) {
          if (err) reject(err);
...
router.post('/:id/items', authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    // Write directly to permanent storage for item creation
    const result = await new Promise((resolve, reject) => {
      storage.permanentDb.run(
        'INSERT INTO game_items (game_id, name, description, price, type) VALUES (?, ?, ?, ?, ?)',
        [req.params.id, name, description, price, type || DEFAULT_GAME_TYPE],
        function(err) {
          if (err) reject(err);
          else resolve({ success: true, id: this.lastID });
        }
