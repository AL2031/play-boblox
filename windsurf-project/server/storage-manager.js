...
`;
        ...
        this.permanentDb.run(
          ...
          ['Boblox Adventure', 'A fun platformer game to get you started!', userId, gameCode, 1],
          function(err) {
            if (err) {
              reject(err);
              return;
  ...
  getReadSql(type, operation) {
    const sqlMap = {
      ...
      'game': {
        'all': 'SELECT g.*, u.username as creator_name, COUNT(DISTINCT gi.id) as item_count FROM games g LEFT JOIN users u ON g.creator_id = u.id LEFT JOIN game_items gi ON g.id = gi.game_id WHERE g.published = 1 GROUP BY g.id',
